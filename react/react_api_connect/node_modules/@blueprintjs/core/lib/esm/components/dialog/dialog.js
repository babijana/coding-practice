...
export class Dialog extends AbstractPureComponent {
    ...
    render() {
        ...
        return (_jsx(Overlay2, { ...overlayProps, className: Classes.OVERLAY_SCROLL_CONTAINER, childRef: this.childRef, hasBackdrop: true, children: _jsx("div", { className: Classes.DIALOG_CONTAINER, ref: mergeRefs(containerRef, this.childRef), children: _jsxs("div", { className: classNames(Classes.DIALOG, className), role: role, "aria-modal": overlayProps.enforceFocus ?? true, "aria-labelledby": this.props["aria-labelledby"] || (title ? this.titleId : undefined), "aria-describedby": this.props["aria-describedby"], style: style, children: [this.maybeRenderHeader(), children] }) }) }));
    }
    validateProps(props) {
        if (props.title == null) {
            if (props.icon != null) {
    ...
    maybeRenderCloseButton() {
        ...
        if (this.props.isCloseButtonShown !== false) {
            return (_jsx(Button, { "aria-label": "Close", className: Classes.DIALOG_CLOSE_BUTTON, icon: _jsx(SmallCross, { size: IconSize.STANDARD }), onClick: this.props.onClose, variant: "minimal" }));
        }
        else {
            return undefined;
        }
    ...
    maybeRenderHeader() {
        ...
        return (_jsxs("div", { className: Classes.DIALOG_HEADER, children: [_jsx(Icon, { icon: icon, size: IconSize.STANDARD, "aria-hidden": true, tabIndex: -1 }), _jsx(TitleTagName, { id: this.titleId, children: title }), this.maybeRenderCloseButton()] }));
    }
}
